...
for split_seed in range(20):
    try:
        ...
        filename = args.save_to+'/results/exp1_'+str(split_seed)+'/'+args.dataset+'_vcr.pkl'
        ...
        if os.path.exists(filename):
            ...
            raw_adj_sp, original_adj, features, cluster_dict = get_VCR_data(graph, features, args.num_structure_supernodes, normalize=args.adj_renorm)
            ...
            processed_features = utils.re_features_push_structure(raw_adj_sp, original_adj, features, args.hops, args.num_structure_tokens, args.num_structure_supernodes, cluster_dict)  # return (N, num_structure_tokens + 1, d)
            ...
            # Model configuration.
            model = TransformerModel(args.hops + args.num_structure_tokens + args.num_content_tokens, 
            ...
            print(model)
            print(TransformerModel.__init__.__doc__)
            ...
            for epoch in range(args.epochs):
                ...
                if loss_val < best_val_loss:
                    ...
                    best_epoch = epoch
                    no_improvement_count = 0
                    torch.save(model.state_dict(), args.save_to+'/results/exp1_'+str(split_seed)+'/best_model_vcr_'+args.dataset+'.pth')
                else:
                    no_improvement_count += 1
                if no_improvement_count >= args.patience:
                    model.load_state_dict(torch.load(args.save_to+'/results/exp1_'+str(split_seed)+'/best_model_vcr_'+args.dataset+'.pth'))
