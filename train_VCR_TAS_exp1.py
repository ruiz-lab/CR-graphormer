...
from adjacency_search import get_auxiliary_graph, get_weights, extract_edge_weights
...
try:
    ...
    if not os.path.exists(filename):
        ...
        if "synthesized" in args.dataset:
            ...
            features = torch.tensor(all_features[i][:len(graph.nodes())], dtype=torch.float32)[:len(graph.nodes())]
        ...
        src = edge_index[0]
        dst = edge_index[1]
        auxiliary_graph = dgl.graph((src, dst), num_nodes=len(graph.nodes())).to('cpu')
        weights = get_weights(AS_object, auxiliary_graph)
        edge_weights = extract_edge_weights(edge_index, weights)
        row, col = auxiliary_graph.edges()
        row = row.cpu().numpy()
        col = col.cpu().numpy()
        edge_indices = torch.from_numpy(np.vstack([row, col])).long()
        raw_adj_sp, original_adj, features, cluster_dict = get_VCR_data(auxiliary_graph, features, args.num_structure_supernodes, args.adj_renorm, edge_weights, row, col)
        ...
        processed_features = utils.re_features_push_structure(raw_adj_sp, original_adj, features, args.hops, args.num_structure_tokens, args.num_structure_supernodes, cluster_dict, edge_weights, edge_indices)  # return (N, num_structure_tokens + 1, d)
        processed_features = utils.re_features_push_content(original_adj, processed_features, features, args.num_content_tokens, num_labels, labelcluster_to_nodes, edge_weights, edge_indices).to(device) # return (N, num_content_tokens + num_structure_tokens + 1, d)
        ...
        del graph, auxiliary_graph, AS_object
        ...
        # Model configuration.
        model = TransformerModel(processed_features.shape[1] - 1, 
