...
import os
import pickle
from adjacency_search import TAS
import dgl
...
if os.path.exists("graph_connectivity_data_TAS_"+args.save_to+".pkl"):
    with open("graph_connectivity_data_TAS_"+args.save_to+".pkl", "rb") as f:
        data = pickle.load(f)
...
for i in range(start,len(datasets)):
    ...
    for split_seed in range(20):
        ...
        # Pre-process data.
        filename_adjacency_search = args.save_to+'/saved_adjacency_search/'+dataset+'_tas.pkl'
    ...
    with open("graph_connectivity_data_TAS_"+args.save_to+".pkl", "wb") as f:
        pickle.dump(data, f)
    ...
    df.to_excel("graph_connectivity_data_TAS_"+args.save_to+".xlsx", index=False)
