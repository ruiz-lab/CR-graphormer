...
import os
import pickle
from adjacency_search import TAS, get_auxiliary_graph, update_score2, update_score3
import dgl
...
for split_seed in range(20):
    ...
    try:
        ...
        filename = args.save_to+'/results/exp2_'+str(split_seed)+'/'+args.dataset+'_vcr_tas.pkl'
        if not os.path.exists(args.save_to):
            os.makedirs(args.save_to)
        if not os.path.exists(args.save_to+'/results'):
            os.makedirs(args.save_to+'/results')
        if not os.path.exists(args.save_to+'/results/exp2_'+str(split_seed)):
            os.makedirs(args.save_to+'/results/exp2_'+str(split_seed))
        ...
        if not os.path.exists(filename):
            ...
            processed_features = utils.re_features_push_structure(raw_adj_sp, original_adj, features, 1, args.num_structure_tokens)  # return (N, num_structure_tokens + 1, d)
            ...
            for epoch in range(args.epochs):
                ...
                if loss_val < best_val_loss:
                    ...
                    best_epoch = epoch
                    no_improvement_count = 0
                    torch.save(model.state_dict(), args.save_to+'/results/exp2_'+str(split_seed)+'/best_model_vcr_tas_'+args.dataset+'.pth')
                else:
                    no_improvement_count += 1
                if no_improvement_count >= args.patience:
                    model.load_state_dict(torch.load(args.save_to+'/results/exp2_'+str(split_seed)+'/best_model_vcr_tas_'+args.dataset+'.pth'))
