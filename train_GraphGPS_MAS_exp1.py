...
for split_seed in range(20):
    ...
    try:
        ...
        if not os.path.exists(filename):
            ...
            lpe = laplacian_positional_encoding(auxiliary_graph, args.pe_dim)
            features = torch.cat((features, lpe), dim=1).float().to(device)
            labels = labels.to(device)
            weights = get_weights(AS_object, auxiliary_graph)
            edge_weights = extract_edge_weights(edge_index, weights).to(device)

            del graph, auxiliary_graph, weights, AS_object

            # Model configuration.
            model = GraphGPS(in_dim=features.shape[1],
            ...
            def train_valid_epoch(epoch,dictionary):
                model.train()
                optimizer.zero_grad()
                output = model(features,edge_index,edge_weights)
