...
for split_seed in range(20):
    ...
    try:
        ...
        filename = args.save_to+'/results/exp1_'+str(split_seed)+'/'+args.dataset+'_nag.pkl'
        ...
        if not os.path.exists(filename):
            ...
            for epoch in range(args.epochs):
                ...
                if loss_val < best_val_loss:
                    ...
                    best_epoch = epoch
                    no_improvement_count = 0
                    torch.save(model.state_dict(), args.save_to+'/results/exp1_'+str(split_seed)+'/best_model_nag_'+args.dataset+'.pth')
                else:
                    no_improvement_count += 1
                if no_improvement_count >= args.patience:
                    model.load_state_dict(torch.load(args.save_to+'/results/exp1_'+str(split_seed)+'/best_model_nag_'+args.dataset+'.pth'))
