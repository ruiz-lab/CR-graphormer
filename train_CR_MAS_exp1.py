from data import get_dataset, get_VCR_data
import time
import utils
import random
import argparse
...
import os
import pickle
from adjacency_search import MAS, get_auxiliary_graph, get_weights, update_score2, update_score3
import dgl
...
for split_seed in range(20):
        ...
        filename = args.save_to+'/results/exp1_'+str(split_seed)+'/'+args.dataset+'_cr_mas.pkl'
        ...
        if not os.path.exists(filename):
                ...
                try:
                    ...
                    try:
                        with open(filename_adjacency_search, 'rb') as f:
                            AS_object = pickle.load(f)
                        if args.frequency_normalization == "global":
                            update_score2(AS_object)
                        elif args.frequency_normalization == "local":
            ...
            processed_features = utils.re_features_push_structure(raw_adj_sp, original_adj, features, 1, args.num_structure_tokens, weights)  # return (N, num_structure_tokens + 1, d)
            processed_features = utils.re_features_push_content(original_adj, processed_features, features, args.num_content_tokens, num_labels, labelcluster_to_nodes, weights) # return (N, num_content_tokens + num_structure_tokens + 1, d)
            ...
            for epoch in range(args.epochs):
                ...
                if loss_val < best_val_loss:
                    ...
                    best_epoch = epoch
                    no_improvement_count = 0
                    torch.save(model.state_dict(), args.save_to+'/results/exp1_'+str(split_seed)+'/best_model_cr_mas_'+args.dataset+'.pth')
                else:
                    no_improvement_count += 1
                if no_improvement_count >= args.patience:
                    model.load_state_dict(torch.load(args.save_to+'/results/exp1_'+str(split_seed)+'/best_model_cr_mas_'+args.dataset+'.pth'))
